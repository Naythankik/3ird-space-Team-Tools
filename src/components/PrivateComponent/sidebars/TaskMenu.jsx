...
import {FaFile, FaHashtag, FaHeadphones, FaTextHeight} from "react-icons/fa";
...
const TaskMenu = () => {
    ...
    return (
        <article
            id="taskOptions"
            className="hidden bg-blend-overlay rounded-lg border border-gray-300 bg-indigo-300 text-black/60 w-88 absolute bottom-2 left-16.5 shadow-lg py-3"
        >
            ...
            <div className="border-b border-gray-300">
                {TASK_OPTIONS.map(({ title, desc, icon: { type: Icon, color } }, idx) => (
                    <Link
                        to="#"
                        key={idx}
                        className="group flex items-center p-3 hover:bg-indigo-200 hover:text-gray-800 transition"
                    >
